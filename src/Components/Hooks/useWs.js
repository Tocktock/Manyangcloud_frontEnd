...
export function useWs() {
			...
			ws.onmessage = function(event) {
				...
				switch(tjo['type']) {
					...
					case "stored-jwt-token-valid":
						setVerifiedJwt(true);
						setJwt(window.localStorage.getItem('ManyangcloudJwt'));
					...
					case "jwt-token-invalid":
						...
						setValidCredentials(null);
						setLoading(false);
						if(window.localStorage.getItem('ManyangcloudJwt') !== null) { window.localStorage.removeItem('ManyangcloudJwt');  }
						break;
    ...
    useEffect(() => {
		if(ws === null) { console.log("configure start"); setWs(new WebSocket('wss://manyangcloud.com:1200/api')); } // [ "master"] 
	...
	useEffect(() => {
	    if (jwt !== '^vAr^' && verifiedJwt) { 
		    ...
		    window.localStorage.setItem('ManyangcloudJwt',jwt); 
